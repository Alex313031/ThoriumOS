...
# Orderfile generate does not use production Orderfile.
REQUIRED_USE="
	cfi? ( thinlto )
...
set_build_args() {
	...
	*)
	...
	if use_remoteexec; then
		...
		mkdir -p /tmp/reclient-chromeos-chrome

		einfo "Starting reproxy"
		"${DEPOT_TOOLS}/reclient/bootstrap" \
			--cfg="/mnt/host/source/chromite/sdk/reclient_cfgs/${REPROXY_CFG_FILENAME}" \
			--re_proxy "${DEPOT_TOOLS}/reclient/reproxy" \
			|| die "Fatal - cannot start reproxy for distributed builds."

		# 3. Fetching reclient's rewrapper configs. This is a chrome specific build step to fetch reclient configs for
		#    sub-portions of the build (eg. building nacl)
		einfo "Fetching rewrapper configs"
		"${CHROME_ROOT}/src/buildtools/reclient_cfgs/fetch_reclient_cfgs.py" \
			--rbe_instance=projects/rbe-chrome-trusted/instances/default_instance \
			|| die "Fatal - fetching reclient cfgs failed. Run fetch_reclient_cfgs.py in chrome checkout to debug."
	fi
...
src_compile() {
	...
	if use_remoteexec; then
		/mnt/host/depot_tools/reclient/bootstrap --shutdown \
			--cfg="/mnt/host/source/chromite/sdk/reclient_cfgs/${REPROXY_CFG_FILENAME}" \
