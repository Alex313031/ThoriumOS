...
set_build_args() {
	...
	*)
	...
	if use_remoteexec; then
		...
		if [[ -z ${SHOULD_USE_REPROXY_CFG_FILE_FOR_CI} ]]; then
			if [[ $(whoami) == "${CHROMEOS_CI_USERNAME}" ]]; then
				SHOULD_USE_REPROXY_CFG_FILE_FOR_CI="true"
				einfo "Will use production reproxy config for user: $(whoami) "
			else
				SHOULD_USE_REPROXY_CFG_FILE_FOR_CI="false"
				einfo "Will use developer reproxy config for user: $(whoami) "
			fi
		else
			einfo "SHOULD_USE_REPROXY_CFG_FILE_FOR_CI is set to ${SHOULD_USE_REPROXY_CFG_FILE_FOR_CI}"
		fi
		if [[ "${SHOULD_USE_REPROXY_CFG_FILE_FOR_CI}" == "true" ]]; then
			REPROXY_CFG_FILENAME="reproxy.cfg"
			einfo "Using production reproxy.cfg config."
		else
			REPROXY_CFG_FILENAME="reproxy_experimental.cfg"
			einfo "Using developer reproxy_experimental.cfg config."
		fi
...
install_chrome_test_resources() {
	...
	echo "Copying Crosier test sudo helpers from: ${CHROME_ROOT}/src/chrome/test/base/chromeos/crosier/helper to ${dest}"
	cp -a -r -f "${CHROME_ROOT}"/src/chrome/test/base/chromeos/crosier/helper/test_sudo_helper.py \
		...
		"${dest}" || die
	echo "Crosier test sudo helpers copied successfully"
}
...
src_install() {
	...
	# Create symlinks so that cros-camera can find those libraries
	if use camera_angle_backend; then
		dosym "${CHROME_DIR}/libEGL.so" "${CHROME_DIR}/libEGL.so.1"
		dosym "${CHROME_DIR}/libGLESv2.so" "${CHROME_DIR}/libGLESv2.so.2"
	fi
}
